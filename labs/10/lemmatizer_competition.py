...
class Network(tf.keras.Model):
    def __init__(self, args, train):
        ...
        self.target_embedding = tf.keras.layers.Embedding(input_dim=self.target_mapping.vocab_size(), output_dim=args.cle_dim)
...
def main(args):
    ...
    model.fit(train, epochs=args.epochs, validation_data=dev, verbose=2,callbacks=[ShowIntermediateResults(dev), model.tb_callback, model_checkpoint_callback])

    # ======================================== Train on new data =======================================================
    # ======================================== Train on new data =======================================================
    from tagger_morpho_dataset import MorphoDataset_tagger

    morpho_tagger = MorphoDataset_tagger("czech_pdt")

    def tagging_dataset(forms, lemmas, tags):
        return forms, lemmas

    train_tagger = morpho_tagger.train.dataset.map(tagging_dataset).apply(tf.data.experimental.dense_to_ragged_batch(args.batch_size))
    dev_tagger = morpho_tagger.dev.dataset.map(tagging_dataset).apply(tf.data.experimental.dense_to_ragged_batch(args.batch_size))
    test_tagger = morpho_tagger.test.dataset.map(tagging_dataset).apply(tf.data.experimental.dense_to_ragged_batch(args.batch_size))

    model.fit(train_tagger, epochs=args.epochs, validation_data=dev_tagger, verbose=2)
